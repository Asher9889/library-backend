...
# # ==========================================
# # LONG-TERM CHAT HISTORY MODELS (SQLite, per-member, date-wise)
# # ==========================================
...
#         # Persist to long-term history too, so "meri photo do" turns show up
#         # in the member's chat history same as any other turn.
#         chat_memory.save_turn_safe(
#             mem_cd=mem_cd,
#             thread_id=request.thread_id,
#             question=user_question,
#             answer=self_photo_response.answer,
...
#     # ---- LONG-TERM MEMORY: persist this finished turn (SQLite, per-member, date-wise) ----
#     # Best-effort: never breaks the response the member is waiting on.
#     chat_memory.save_turn_safe(
#         mem_cd=mem_cd,
#         thread_id=request.thread_id,
#         question=user_question,
...
#     """
#     Clears SHORT-TERM conversation memory (the follow-up resolver's
#     chat_history) for a given thread_id by writing an empty chat_history
#     into the checkpoint. Call this when the user starts a brand-new chat
#     session in the frontend, so old follow-up context doesn't leak in.

#     NOTE: this does NOT touch the long-term, per-member SQLite history —
#     that's permanent and browsable via the /history/... endpoints below.
#     Use DELETE /history/{mem_cd} if you want to wipe that too.
#     """
#     try:
#         library_agent.update_state(
#             config={"configurable": {"thread_id": thread_id}},
...
# # ==========================================
# # 5c. LONG-TERM CHAT HISTORY ENDPOINTS (SQLite, per-member, date-wise)
# # ==========================================
# # These let a logged-in member browse everything they've ever asked,
# # grouped by the date they asked it (in IST), independent of thread_id
# # and surviving server restarts (unlike the LangGraph checkpointer above).
# #
# # SECURITY: `mem_cd` here is taken from the URL path, exactly like the
# # rest of this API takes it from the request body. In production, make
# # sure your frontend/gateway only ever calls these with the CALLER's own
# # authenticated mem_cd (e.g. derived from the RFID/face login session),
# # not an arbitrary value a client could type in — otherwise one member
# # could read another member's chat history.

# @app.get("/history/{mem_cd}/dates", response_model=List[ChatDateSummary])
# def history_dates(mem_cd: str):
#     """List of dates this member has chatted on (newest first), each with
#     a message count — render as a date list / calendar in the UI."""
#     return chat_memory.get_chat_dates(mem_cd)


# @app.get("/history/{mem_cd}/date/{chat_date}", response_model=List[ChatMessageOut])
# def history_for_date(mem_cd: str, chat_date: str):
#     """Full conversation for one member on one day (YYYY-MM-DD, IST),
#     oldest first — this is what the member sees after tapping a date."""
#     rows = chat_memory.get_messages_for_date(mem_cd, chat_date)
#     return [_row_to_message_out(r) for r in rows]


# @app.get("/history/{mem_cd}", response_model=List[ChatMessageOut])
# def history_all(mem_cd: str, limit: int = 50, offset: int = 0):
#     """Paginated full history, newest first — for an infinite-scroll
#     'all my chats' view instead of browsing date by date."""
#     rows = chat_memory.get_all_messages(mem_cd, limit=limit, offset=offset)
#     return [_row_to_message_out(r) for r in rows]


# @app.get("/history/{mem_cd}/search", response_model=List[ChatMessageOut])
# def history_search(mem_cd: str, q: str, limit: int = 50):
#     """Substring search across this member's own past questions + answers."""
#     rows = chat_memory.search_messages(mem_cd, q, limit=limit)
#     return [_row_to_message_out(r) for r in rows]


# @app.delete("/history/{mem_cd}")
# def history_clear(mem_cd: str):
#     """Wipe a member's own long-term chat history (e.g. a 'Clear my
#     history' button in the app, or a data-deletion request)."""
...
import chat_memory
# === TOKEN TRACKING (works with any OpenAI-compatible chat model, incl. Ollama) ===
...
class NodeTokenTracker(BaseCallbackHandler):
    ...
    usage via llm_output.token_usage / generation_info.token_usage (this is
    the shape used by ChatOpenAI, and therefore also by Ollama's
    OpenAI-compatible /v1 endpoint via ChatOpenAI). Accumulates per-node
    totals using 'metadata.node_name' set on each LLM invoke.
    """
    def __init__(self):
        super().__init__()
        self._lock = threading.Lock()
        self.usage = defaultdict(lambda: {"prompt": 0, "completion": 0, "total": 0, "calls": 0})
        self.grand_total = {"prompt": 0, "completion": 0, "total": 0, "calls": 0}
        # run_id -> node_name, captured at on_llm_start. Needed because some
        # LangChain callback managers do NOT forward `metadata` to
        # on_llm_end (only to on_llm_start), which was silently bucketing
        # every call into "unknown" regardless of which node made it.
        self._run_node_map: Dict[str, str] = {}

    def _extract_usage(self, response):
        # Shape 1: llm_output.token_usage  (batched / single invoke)
        ...
        if isinstance(llm_output, dict):
            for key in ("token_usage", "usage"):
                ...
                if u:
                    ...
                    if t > 0:
                        return p, c, t
        # Shape 2: generations[i][j].generation_info.token_usage  (common alt shape)
        ...
        for batch in gens:
            for g in batch:
                ...
                if u:
                    ...
                    if t > 0:
                        return p, c, t
                # Shape 3: message.usage_metadata (ChatOpenAI / ChatOllama AIMessage)
    ...
    def on_llm_start(self, serialized, prompts, *, run_id=None, parent_run_id=None,
                     tags=None, metadata=None, **kwargs):
        """Reliably capture which node fired this call. metadata IS passed
        here by LangChain's callback manager (unlike on_llm_end, where it's
        often missing), so we stash node_name keyed by run_id and look it
        up again in on_llm_end."""
        node = "unknown"
        if metadata:
            node = metadata.get("node_name") or metadata.get("node") or (tags[0] if tags else "unknown")
        elif tags:
            node = tags[0]
    ...
    def summary(self) -> str:
        lines = [
            "\n" + "=" * 76,
            "🧪 LLM TOKEN USAGE SUMMARY (per LangGraph node, per-node LLM instances) — Ollama/Qwen",
            "=" * 76,
            f"{'Node':<28}{'Calls':>7}{'Prompt':>12}{'Completion':>14}{'Total':>12}",
            "-" * 76,
        ]
        with self._lock:
            for node, u in sorted(self.usage.items(), key=lambda x: -x[1]["total"]):
                lines.append(f"{node:<28}{u['calls']:>7}{u['prompt']:>12}{u['completion']:>14}{u['total']:>12}")
            lines.append("-" * 76)
            gt = self.grand_total
            lines.append(f"{'GRAND TOTAL':<28}{gt['calls']:>7}{gt['prompt']:>12}{gt['completion']:>14}{gt['total']:>12}")
            lines.append("=" * 76 + "\n")
        return "\n".join(lines)

# Single shared instance. Reset per request.
...
# ==========================================
# NOTE: Move these to real environment variables / a secrets manager before
# deploying. Hardcoded DB credentials in source is a security risk,
# especially since this file will end up in git history / logs.

# ---- Local LLM (Ollama / Qwen) configuration ----
# We no longer call any external LLM API (Groq removed). Instead we talk to
# a locally-hosted Ollama server exposing an OpenAI-compatible /v1 endpoint,
# serving a local Qwen model on the GPU. Configure via .env:
#   OLLAMA_BASE_URL=http://127.0.0.1:11434/v1
#   OLLAMA_MODEL=qwen3.6:27b
OLLAMA_BASE_URL = os.environ.get("OLLAMA_BASE_URL", "http://127.0.0.1:11434/v1")
OLLAMA_MODEL = os.environ.get("OLLAMA_MODEL", "")
# Ollama's OpenAI-compatible endpoint does not validate the API key, but the
# OpenAI SDK requires a non-empty string to be set, so we pass a dummy value.
...
# ==========================================
# LOCAL LLM CLIENTS (Ollama, OpenAI-compatible /v1 endpoint)
# ==========================================
# Ollama exposes an OpenAI-compatible Chat Completions API at
# {OLLAMA_BASE_URL}/chat/completions, so we can reuse langchain_openai's
# ChatOpenAI client unchanged, just pointed at the local server instead of
# api.openai.com.
#
# PER-NODE LLM INSTANCES (perf/token optimization)
# --------------------------------------------------
# Each LangGraph node does a fundamentally different job with a very
# different expected OUTPUT size:
#   - resolve_followup : rewrites one short question       -> tiny output
#   - generate_sql      : needs the full schema/rules prompt -> larger input,
#                          but the output is one SQL statement
#   - generate_answer   : formats DB rows into markdown      -> can be a
#                          longer output (tables with many rows)
#
# Giving each its own ChatOpenAI instance lets us cap `max_tokens` per task
# instead of one global, overly generous limit. This alone speeds up local
# Ollama generation noticeably (it doesn't have to allocate/consider a huge
# max output window for a call that's really just rewriting one sentence),
# and caps runaway generations. The *input* prompt each node sends was
# already scoped correctly (resolve_followup/generate_answer never sent the
# big SYSTEM_PROMPT — only generate_sql does), so this change is about
# output-side limits and making that separation explicit/impossible to
...
# Resolver: output is just one rewritten question, keep it short & fast.
llm_resolver = ChatOpenAI(**_COMMON_LLM_KWARGS, max_tokens=200)

# SQL generator: needs the big schema/rules SYSTEM_PROMPT as input, but the
# output is a single T-SQL statement (even a complex one rarely needs more
# than a few hundred tokens).
llm_sql = ChatOpenAI(**_COMMON_LLM_KWARGS, max_tokens=800)

# Answer formatter: can legitimately need to emit large markdown tables
# (up to MAX_ROWS=100 rows with S.No. + several columns), so give it more
# room than the other two.
llm_answer = ChatOpenAI(**_COMMON_LLM_KWARGS, max_tokens=2000)

# Back-compat alias: a few places (startup ping, /health/llm) just need
# *some* working model to verify Ollama connectivity — reuse llm_sql for
# that rather than spinning up a fourth client.
llm = llm_sql
...
def resolve_followup_node(state: AgentState):
    ...
    try:
        response = llm_resolver.invoke([HumanMessage(content=prompt)], config={
        "callbacks": [token_tracker],
        "metadata": {"node_name": "resolve_followup"},
        },)
        resolved = (response.content or "").strip().strip('"').strip("'").strip()
        if not resolved:
            resolved = question
...
def generate_sql_node(state: AgentState):
    ...
    response = llm_sql.invoke(messages, config={
        "callbacks": [token_tracker],
...
def generate_answer_node(state: AgentState):
    print("---GENERATING FINAL ANSWER---")
    messages = build_generate_answer_messages(state)
    response = llm_answer.invoke(messages, config={
        "callbacks": [token_tracker],
        "metadata": {"node_name": "generate_answer"},
    },)
    return {"query_result": response.content}
...
@app.get("/")
def read_root():
    return {"status": f"SOUL30 Library Text-to-SQL API v3.4 (Ollama/{OLLAMA_MODEL}, per-node LLM instances, follow-up memory + long-term chat history) is running perfectly!"}


@app.get("/health/llm")
def health_llm():
    """Quick check that the local Ollama server is reachable and the
    configured model responds. Useful for kiosk/frontend startup checks."""
    try:
        resp = llm.invoke([HumanMessage(content="ping")], config={"metadata": {"node_name": "health_check"}})
        return {
            "status": "ok",
            "provider": "ollama",
            "base_url": OLLAMA_BASE_URL,
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Local Ollama LLM unreachable at {OLLAMA_BASE_URL} (model={OLLAMA_MODEL}): {e}",
        )
...
def _is_llm_connection_error(err: Exception) -> bool:
    """Best-effort detection of 'local Ollama server unreachable / down'
    errors so we can show a friendly bilingual message instead of a raw
    stack trace. Covers httpx/requests connection errors raised by the
    OpenAI SDK when it can't reach OLLAMA_BASE_URL."""
    msg = str(err).lower()
    needles = (
...
def ask_library_agent(request: QueryRequest):
    ...
    # 🧪 Har request se pehle tracker reset karo taaki pichli request ka data mix na ho
    token_tracker.reset()
    ...
    try:
        final_state = library_agent.invoke(
            ...
            config={
                ...
                "callbacks": [token_tracker],   # 🧪 safety net: catches any LLM call inside nodes
            },
        )
    except Exception as e:
        if _is_llm_connection_error(e):
            ...
            return QueryResponse(
                ...
                resolved_question=user_question,
                sql_query="",
                answer="स्थानीय AI मॉडल (Ollama) से कनेक्ट नहीं हो पाया। कृपया सुनिश्चित करें कि Ollama server "
                       f"({OLLAMA_BASE_URL}) चालू है और मॉडल '{OLLAMA_MODEL}' pull किया हुआ है। "
                       "Could not connect to the local Ollama LLM server — please make sure it's running.",
                attempts=1,
    ...
    # 🧪 Graph execution ke baad, token usage summary print karo
...
async def _ask_library_agent_v2_events(request: QueryRequest):
    ...
    try:
        ...
        if _is_llm_connection_error(e):
            ...
            yield _sse(
                "token",
                text="स्थानीय AI मॉडल (Ollama) से कनेक्ट नहीं हो पाया। कृपया सुनिश्चित करें कि Ollama server "
                     f"({OLLAMA_BASE_URL}) चालू है और मॉडल '{OLLAMA_MODEL}' pull किया हुआ है। "
                     "Could not connect to the local Ollama LLM server — please make sure it's running.",
            )
            yield _sse(
    ...
    full_answer_parts: List[str] = []
    try:
        async for chunk in llm_answer.astream(messages, config={
        "callbacks": [token_tracker],
        ...
        if _is_llm_connection_error(e):
            ...
            yield _sse(
                "error",
                message=f"Could not connect to the local Ollama LLM server ({OLLAMA_BASE_URL}). "
...
async def startup_event():
    ...
    try:
        llm.invoke([HumanMessage(content="ping")], config={"metadata": {"node_name": "startup_ping"}})
        print("[SYSTEM] Ollama LLM reachable ✅")
    except Exception as e:
        print(f"[SYSTEM WARNING] Ollama LLM NOT reachable at startup ({OLLAMA_BASE_URL}): {e}")
