...
# ==========================================
# 3b. ASR CORRECTION PROMPT
# ==========================================
ASR_CORRECTION_PROMPT = """You are an advanced Speech-to-Text (ASR) Error Corrector for a Library Management Voice Bot.
...
class FaceEmbeddingCache:
    ...
    def refresh_embeddings(cls):
        ...
        if current_time - cls._last_refresh < 120: return
...
def correct_asr_errors_node(state: AgentState):
    ...
    if not history_str: history_str = "No history"
    
    prompt = ASR_CORRECTION_PROMPT.format(history=history_str, question=question)
    
    try:
        response = llm_resolver.invoke(
            [HumanMessage(content=prompt)], 
            config={
                "callbacks": [token_tracker],
                "metadata": {"node_name": "correct_asr_errors"}
            }
        )
        ...
        if not corrected or len(corrected) > len(question) * 3:
            print(f"[ASR] LLM returned invalid response. Falling back to original.")
            corrected = question
...
def generate_answer_node(state: AgentState):
    ...
    if state.get("error") and not state.get("report_data"):
        return {"query_result": "Sorry, I couldn't fetch the data due to a database error."}
...
stream_workflow.add_node("correct_asr_errors", correct_asr_errors_node)
...
stream_workflow.set_entry_point("correct_asr_errors")
stream_workflow.add_edge("correct_asr_errors", "resolve_followup")
...
def welcome_api():
    return {
        ...
        "sample_questions": ["Library me total kitni physical books hain?"]
    }
...
def try_answer_self_photo(user_question: str, mem_cd: str) -> Optional["QueryResponse"]:
    ...
    if not photo:
        return QueryResponse(
            ...
            resolved_question=user_question,
            sql_query="",
            answer="आपकी फोटो अभी तक रजिस्टर नहीं है.",
            attempts=0,
            report_available=False,
            report_id=None,
            image_data=None,
