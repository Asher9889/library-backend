...
MAX_ROWS = 20   # LLM ko 20 rows bhejne kaafi hain
MAX_RETRIES = 5
_conn = None
...
# IMPORTANT: LangGraph app ko fastapi app se alag naam dena hai
...
# 5. FASTAPI APP SETUP
# ==========================================
app = FastAPI(title="Library Text-to-SQL API", version="1.0")
...
@app.get("/")
def read_root():
    return {"status": "Library Text-to-SQL API is running!"}

@app.post("/ask", response_model=QueryResponse)
def ask_library_agent(request: QueryRequest):
    """
    FastAPI endpoint jo user ka sawal lega, LangGraph agent ko dega,
    aur final answer return karega.
    """
    ...
    # Extracting results safely
    final_answer = final_state.get("query_result", "Agent failed to generate answer.")
    executed_sql = final_state.get("sql_query", "No SQL generated.").replace("```sql", "").replace("```", "").strip()
